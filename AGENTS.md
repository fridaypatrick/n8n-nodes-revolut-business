...
- **Signature Verification**: Auto-register mode always verifies incoming signatures using the Revolut-generated signing secret stored in workflow static data at activation time. Manual mode (auto-register disabled) uses the Verify Signature toggle and the Signing Secret field. Verification depends on raw request body access and the `Revolut-Request-Timestamp` header; fails closed if unavailable. The verifier follows Revolut's `v1.{timestamp}.{rawBody}` HMAC-SHA256 payload format; implementation details are in `src/helpers/webhookSignature.ts`.
